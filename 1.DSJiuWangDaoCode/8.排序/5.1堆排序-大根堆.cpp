...
void Down(int arr[], int n, int k){// n是有多少个元素，i是当前第几个下坠 
	...
	for(int i = k * 2; i <= n; i *= 2){// 初始i为k的左结点， 每次增长2倍是不断的往子树下面下滤 
		if(i + 1 <= n && arr[i] < arr[i + 1]){// 先对左右结点对比，选出更大结点的下标 
			i++;
		}
		// !---这里写错过--------- 
		if(arr[0] > arr[i]){// 如果父结点大于左右结点表示满足大根堆就退出，记住需要用保存的arr[0],因为k的值会变 
			break; 
		}
...
void HeapSort(int arr[], int n){
	// 每次选出根结点元素的值再保持大根堆，直至n - 1个就是堆排序 ，最后一个不用排序了 
