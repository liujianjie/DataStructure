...
void FindParent1(BiTree p, int val){
	if(parents != NULL){
		return;
	}
	if(p->lchild != NULL){
		if(p->lchild->data == val){
			parents = p;
		}
		FindParent1(p->lchild, val);
	}
	if(p->rchild != NULL){
		if(p->rchild->data == val){
			parents = p;
		}
		FindParent1(p->rchild, val);
	}
} 
void FindParent2(BiTree p, int val, BiTNode *last){
	if(p == NULL){
		return;
	}
	if(p->data == val){
	...
	FindParent2(p->lchild, val, p);
	FindParent2(p->rchild, val, p);
} 
...
//	FindParent1(root, 4);
	FindParent2(root, 4, NULL);
	if(parents != NULL){
		cout<< "父亲："<< parents->data << "" << endl;
	}else{
		cout<<"未找到"<<endl; 
	}
