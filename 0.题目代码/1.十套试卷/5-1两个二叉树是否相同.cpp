...
int count1 = 0;
int count2 = 0;
bool IsSame(BiTree p1, BiTree p2){// p是当前节点，e是插入值 
	count1++;
	if(p1 == NULL && p2 == NULL){
		return true;
	}
...
void IsSame2(BiTree p1, BiTree p2){// p是当前节点，e是插入值 
	count2++;
	if(!allargument){
		return;
	}
...
int main(){
	...
	cout<<"是否相等:"<<boolalpha <<b<<endl;
	cout<<count1<<endl;
	
	IsSame2(root, root2);	
	cout<<"是否相等:"<<boolalpha <<allargument<<endl;
	cout<<count2<<endl;
